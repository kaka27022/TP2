...
Posicao primeiraPosicaoPilha(Labirinto *labirinto) {
    Posicao firstpos;
...
int acharSaidaPilha(Labirinto *labirinto) {
    ...
    iniciaPilha(pilha);
    Posicao pos = primeiraPosicaoPilha(labirinto);
...
void pilhaPush(Pilha *pilha, Posicao pos) {
    Celulap *new = (Celulap*)malloc(sizeof(Celulap));
    if (new == NULL)
...
void desalocarPilha(Pilha *pilha) {
    Celulap *aux = pilha->cabeca;
    Celulap *aux2 = pilha->cabeca->prox;
    while(aux2 != NULL) {
        free(aux);
        aux = aux2;
        aux2 = aux2->prox;
